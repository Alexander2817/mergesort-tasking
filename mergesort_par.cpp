...
const size_t threshold = 15000;
void par_mergesort(int* arr, size_t l, size_t r, int* temp) {
    if (l>=r) 
        return;
    if (r-l+1 <= threshold) { // if smaller than threshold run sequential
    ...
    //If true, create left thread
    if(mid-l > threshold) {
        leftThread = std::thread(par_mergesort, arr, l, mid, new int[r-l+1]);
    } else {
        mergesort(arr, l, mid, temp);
    }
    //If true, create right thread
    if(r-mid > threshold) {
        rightThread = std::thread(par_mergesort, arr, mid+1, r, new int[r-mid+1]);
    } else {
        mergesort(arr, mid+1, r, temp);
    }
...
int main (int argc, char* argv[]) {
  ...
  // sort
  par_mergesort(&(arr[0]), 0, n-1, &(temp[0]));
